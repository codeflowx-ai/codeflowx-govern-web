...
export async function GET(request: NextRequest) {
  ...
  const response = await fetch(`${gatewayUrl}/web/api/v1/governance/data/datasets${queryString ? `?${queryString}` : ''}`, {
    headers: {
...
export async function POST(request: NextRequest) {
  ...
  const response = await fetch(`${gatewayUrl}/web/api/v1/governance/data/datasets`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
