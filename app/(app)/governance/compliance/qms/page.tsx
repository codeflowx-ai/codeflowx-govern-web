...
export default function QmsPage() {
  ...
  const getComplianceStatusBadge = (status: string) => {
    switch (status) {
      case "COMPLIANT":
        return <Badge className="bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300">{t("governance.qms.statusValues.compliant", "Cumple")}</Badge>;
      case "PARTIAL":
        return <Badge className="bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300">{t("governance.qms.statusValues.partial", "Parcial")}</Badge>;
      default:
        return <Badge variant="danger">{t("governance.qms.statusValues.nonCompliant", "No Cumple")}</Badge>;
    }
  };
