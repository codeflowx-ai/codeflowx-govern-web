...
export default function AutoApprovalPage() {
  ...
  const [actionDecisionId, setActionDecisionId] = useState<string | null>(null);
  ...
  const handleManualReview = async (decisionId: string) => {
    try {
      setActionDecisionId(decisionId);
      await governanceService.requireManualReview({
        decisionId,
        reason: 'Manual review requested by user'
      });
      // Update optimista: PENDING -> HUMAN_REVIEW, para habilitar aprobar/rechazar
        ...
        prev.map((d) =>
          d.id === decisionId
            ? {
              ...d,
              status: 'human_review',
              reason: 'Revisión manual solicitada por usuario'
            }
            : d
        )
      );
      toast({
        ...
        description: 'Se envió a revisión humana y quedó disponible para aprobar/rechazar.',
        variant: 'info'
      });
    } catch (error) {
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'No se pudo solicitar revisión manual',
        variant: 'error'
      });
    } finally {
      setActionDecisionId(null);
    }
  };
  const handleApprove = async (decisionId: string) => {
    try {
      ...
      // Update optimista: se elimina de la cola de pendientes
      setPendingDecisions((prev) => prev.filter((d) => d.id !== decisionId));
      toast({
        title: 'Aprobado',
        description: 'La decisión fue aprobada correctamente.',
        variant: 'success'
      });
    } catch (error) {
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'No se pudo aprobar la decisión',
        variant: 'error'
      });
    } finally {
      setActionDecisionId(null);
  ...
  const handleReject = async (decisionId: string) => {
    try {
      ...
      toast({
        title: 'Rechazado',
        description: 'La decisión fue rechazada correctamente.',
        variant: 'warning'
      });
    } catch (error) {
      ...
      toast({
        title: 'Error',
        description: error instanceof Error ? error.message : 'No se pudo rechazar la decisión',
        variant: 'error'
      });
    } finally {
      setActionDecisionId(null);
      ...
      <Card>
        ...
        <CardBody>
          <div className="h-64 flex items-end justify-between gap-1">
            {approvalData?.chartData.map((day) => {
                      ...
                      {decision.status === 'human_review' && (
                        <>
                          <Button
                            ...
                            disabled={actionDecisionId === decision.id}
