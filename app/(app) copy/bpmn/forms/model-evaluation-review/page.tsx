...
// Migrado de: model-evaluation-review-form.zul
'use client'
...
function ModelEvaluationReviewFormContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.error || t('modelEvaluationReview.errors.submitError', 'Error al procesar el formulario'))
      }
    } catch (error) {
      console.error('Error procesando formulario:', error)
      alert(t('modelEvaluationReview.errors.submitError', 'Error al procesar el formulario'))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Button variant="outline" size="sm" onClick={handleCancel}>
          ...
          {t('modelEvaluationReview.back', 'Volver')}
        </Button>
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Target className="w-6 h-6 text-primary" />
            {t('modelEvaluationReview.title', 'Título del Formulario')}
          </h1>
          <p className="text-muted-foreground">
            {t('modelEvaluationReview.subtitle', 'Descripción del formulario')}
          </p>
        </div>
      </div>
      ...
      <Card className="group hover:scale-[1.01] transition-all duration-700 bg-card/80 backdrop-blur-sm border-border shadow-lg">
        <CardHeader>
          <CardTitle>
            {t('modelEvaluationReview.formTitle', 'Datos del Formulario')}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-2 justify-end pt-4 border-t">
              <Button type="button" variant="outline" onClick={handleCancel} disabled={loading}>
                {t('modelEvaluationReview.cancel', 'Cancelar')}
              </Button>
              <Button type="submit" disabled={loading}>
                <Save size={16} className="mr-2" />
                {loading
                  ? t('modelEvaluationReview.saving', 'Guardando...')
                  : t('modelEvaluationReview.submit', 'Enviar')}
              </Button>
            </div>
          </form>
        </CardContent>
...
export default function ModelEvaluationReviewPage() {
  return (
    <Suspense fallback={
      ...
      <ModelEvaluationReviewFormContent />
    </Suspense>
  )
}
