// app/(app)/bpmn/forms/categorize-incident/page.tsx
...
function CategorizeIncidentFormContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.error || t('categorizeIncident.errors.submitError', 'Error al procesar el formulario'))
      }
    } catch (error) {
      console.error('Error procesando formulario:', error)
      alert(t('categorizeIncident.errors.submitError', 'Error al procesar el formulario'))
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        <Button variant="outline" size="sm" onClick={handleCancel}>
          ...
          {t('categorizeIncident.back', 'Volver')}
        </Button>
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2">
            <Tag className="w-6 h-6 text-primary" />
            {t('categorizeIncident.title', 'Título del Formulario')}
          </h1>
          <p className="text-muted-foreground">
            {t('categorizeIncident.subtitle', 'Descripción del formulario')}
          </p>
        </div>
      </div>
      ...
      <Card className="group hover:scale-[1.01] transition-all duration-700 bg-card/80 backdrop-blur-sm border-border shadow-lg">
        <CardHeader>
          <CardTitle>
            {t('categorizeIncident.formTitle', 'Datos del Formulario')}
          </CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-2 justify-end pt-4 border-t">
              <Button type="button" variant="outline" onClick={handleCancel} disabled={loading}>
                {t('categorizeIncident.cancel', 'Cancelar')}
              </Button>
              <Button type="submit" disabled={loading}>
                <Save size={16} className="mr-2" />
                {loading
                  ? t('categorizeIncident.saving', 'Guardando...')
                  : t('categorizeIncident.submit', 'Enviar')}
              </Button>
            </div>
          </form>
        </CardContent>
...
export default function CategorizeIncidentPage() {
  return (
    <Suspense fallback={
