...
export default function BPMNProcessesPage() {
  ...
  const loadProcesses = async () => {
    try {
      ...
      // Intentar primero con la API local (modo demo)
      let response = await fetch('/api/bpmn/processes/local')

      // Si falla, intentar con la API del backend
      if (!response.ok) {
        console.log('API local no disponible, intentando con backend...')
        response = await fetch('/api/bpmn/processes')
      }
  ...
  const copyToTemporary = async (processId: string) => {
    try {
      ...
      try {
        ...
        console.log('No se pudo cargar desde local, intentando con backend...')
      }

      // Si no está en local, intentar con el backend
      const response = await fetch(`/api/bpmn/processes/${encodeURIComponent(processId)}/copy`, {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to copy process')
      }

      const result = await response.json()
      setCopiedProcesses(prev => new Set([...prev, result.tempId || processId]))

      // Redirigir al diseñador con el proceso temporal
      router.push(`/bpmn-designer?process=${encodeURIComponent(result.tempId || processId)}`)
    } catch (err) {
