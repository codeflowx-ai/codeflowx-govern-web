            ...
            Messagebox.show("¿Está seguro de eliminar este registro?", 
                ...
                event -> {
                    if (Messagebox.ON_YES.equals(event.getName())) {
                        try {
                            businessService.removeFromID(EndpointAnalytics.class, itemId);
                            log.info("Registro eliminado: ID={}", itemId);
                            logActivity("BORRAR", "V_ENDPOINT_ANALYTICS", itemId, "Eliminado registro ID: " + itemId);
                            loadData();
