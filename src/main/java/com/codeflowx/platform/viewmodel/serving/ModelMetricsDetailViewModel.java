...
/**
 * ViewModel para DETALLE/EDICIÓN/CREACIÓN de ModelMetrics
 */
@Slf4j
@Getter
@Setter
@VariableResolver(DelegatingVariableResolver.class)
public class ModelMetricsDetailViewModel extends BaseFront<ModelMetricsDetailViewModel>{
    ...
    public void afterCompose(@ContextParam(ContextType.VIEW) Component view) throws Exception {
        ...
        if ("NEW".equals(mode)) {
            ...
            loadItem(idxmodelmetrics);
        } else {
            log.error("Modo inválido o falta idxmodelmetrics");
    ...
    private boolean validateRequiredFields() {
        ...
        if (currentModelMetrics.getSrvmetricname() != null && currentModelMetrics.getSrvmetricname().length() > 255) {
            errors.append("- Srvmetricname no puede exceder 255 caracteres\n");
        }
        if (currentModelMetrics.getSrvmetricvalue() == null) {
            errors.append("- Srvmetricvalue\n");
        }
        if (currentModelMetrics.getSrvmetrictype() == null || currentModelMetrics.getSrvmetrictype().trim().isEmpty()) {
            errors.append("- Srvmetrictype\n");
        }
        if (currentModelMetrics.getSrvmetricperiod() == null || currentModelMetrics.getSrvmetricperiod().trim().isEmpty()) {
            errors.append("- Srvmetricperiod\n");
        }
        if (currentModelMetrics.getSrvmetrictimestamp() == null) {
            errors.append("- Srvmetrictimestamp\n");
        }
    ...
    public void cancelEdit() {
        ...
        Map<String, Object> params = new HashMap<>();
        params.put("dataParam", idxmodelmetrics);
        params.put("action", Action.LOAD);
        appendPage("plataforma/serving/serving-overview.zul", page.getFellow(IDDESKTOP), params);
    }
    ...
    public void destroy() {
        ...
        try {
            ...
            if (availableSrvmetrictypes != null) {
                ...
                availableSrvmetrictypes = null;
            }
            if (availableSrvmetricperiods != null) {
