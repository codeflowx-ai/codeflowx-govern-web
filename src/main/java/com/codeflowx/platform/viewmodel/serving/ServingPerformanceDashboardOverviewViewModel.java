    ...
    public void loadData() {
        try {
            ...
            pageResult = businessService.findAllView(
                ServingPerformanceDashboard.class,
                pageParams,
                criterias
            );
            ...
            Messagebox.show("¿Está seguro de eliminar este registro?", 
                ...
                event -> {
                    if (Messagebox.ON_YES.equals(event.getName())) {
                        try {
                            businessService.removeFromID(ServingPerformanceDashboard.class, itemId);
                            log.info("Registro eliminado: ID={}", itemId);
                            logActivity("BORRAR", "V_SERVING_PERFORMANCE_DASHBOARD", itemId, "Eliminado registro ID: " + itemId);
                            loadData();
