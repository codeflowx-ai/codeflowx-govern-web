...
/**
 * ViewModel para búsqueda y listado de AnalyticsMetric
...
public class AnalyticsMetricOverviewViewModel extends BaseFront<AnalyticsMetricOverviewViewModel>{
    ...
    private String anlmetrictypeFilter = "ALL";
    private String anlstatusFilter = "ALL";

    // ========== Datos ==========
    private List<AnalyticsMetric> filteredItems = new ArrayList<>();
    ...
    public void loadData() {
        try {
            ...
            pageResult = analyticsMetricService.findAll(pageParams, criterias);
            ...
            if (pageResult != null && pageResult.getContent() != null) {
                ...
                // Auditar búsqueda
                logActivity("BUSCAR", "ANLANALYTICSMETRICS", null,
    ...
    private Criterias buildCriterias() {
        ...
        if (searchTerm != null && !searchTerm.trim().isEmpty()) {
            Criteria criteria = new Criteria(Operation.AND, Evaluation.LIKE, "anlmetricname");
        ...
        if (!"ALL".equals(anlmetrictypeFilter)) {
            Criteria criteria = new Criteria(Operation.AND, Evaluation.EQUALS, "anlmetrictype");
            criteria.setValues(new Object[]{anlmetrictypeFilter});
            criterias.addCriteria(criteria);
        }
    ...
    private void loadGlobalMetrics() {
        try {
            ...
            // Ejemplo: List<AnalyticsMetricMetricsSummary> metrics = businessService.findAllView(AnalyticsMetricMetricsSummary.class);
    ...
    public void registerItem() {
        ...
        appendPage("plataforma/analytics/analytics-detail.zul", page.getFellow(IDDESKTOP), params);
    }
            ...
            Messagebox.show("¿Está seguro de eliminar este registro?",
                ...
                event -> {
                    if (Messagebox.ON_YES.equals(event.getName())) {
                        try {
                            analyticsMetricService.deleteById(itemId);
                            log.info("Registro eliminado: ID={}", itemId);
                            logActivity("BORRAR", "ANLANALYTICSMETRICS", itemId, "Eliminado registro ID: " + itemId);
                            loadData();
