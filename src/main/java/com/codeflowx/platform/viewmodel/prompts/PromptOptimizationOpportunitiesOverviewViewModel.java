    ...
    public void loadData() {
        try {
            ...
            pageResult = businessService.findAllView(
                PromptOptimizationOpportunities.class,
                pageParams,
                criterias
            );
            ...
            Messagebox.show("¿Está seguro de eliminar este registro?", 
                ...
                event -> {
                    if (Messagebox.ON_YES.equals(event.getName())) {
                        try {
                            businessService.removeFromID(PromptOptimizationOpportunities.class, itemId);
                            log.info("Registro eliminado: ID={}", itemId);
                            logActivity("BORRAR", "V_PROMPT_OPTIMIZATION_OPPORTUNITIES", itemId, "Eliminado registro ID: " + itemId);
                            loadData();
