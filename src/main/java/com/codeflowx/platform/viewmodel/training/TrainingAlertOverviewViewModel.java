...
/**
 * ViewModel para búsqueda y listado de TrainingAlert
...
public class TrainingAlertOverviewViewModel extends BaseFront<TrainingAlertOverviewViewModel>{
  ...
  private PageResult<TrainingAlert> pageResult;
  ...
  // ========== Datos ==========
  private List<TrainingAlert> filteredItems = new ArrayList<>();
  ...
  public void loadData() {
    try {
      ...
      pageResult = trainingAlertService.findAll(pageParams, criterias);
      ...
      if (pageResult != null && pageResult.getContent() != null) {
        ...
        // Auditar búsqueda
        logActivity("BUSCAR", "TRNTRAININGALERTS", null,
  ...
  private Criterias buildCriterias() {
    ...
    if (searchTerm != null && !searchTerm.trim().isEmpty()) {
      Criteria criteria = new Criteria(Operation.AND, Evaluation.LIKE, "trnseverity");
  ...
  private void loadGlobalMetrics() {
    try {
      ...
      // Ejemplo: List<TrainingAlertMetricsSummary> metrics =
      // businessService.findAllView(TrainingAlertMetricsSummary.class);
      ...
      Messagebox.show("¿Está seguro de eliminar este registro?",
          ...
          event -> {
            if (Messagebox.ON_YES.equals(event.getName())) {
              try {
                trainingAlertService.deleteById(itemId);
                log.info("Registro eliminado: ID={}", itemId);
                logActivity("BORRAR", "TRNTRAININGALERTS", itemId, "Eliminado registro ID: " + itemId);
                loadData();
   ...
   * @param mensaje -- mensaje aclaratorio, ejemplo ha cre
