    ...
    public void loadData() {
        try {
            ...
            pageResult = businessService.findAllView(
                MetricSeriesVisualization.class,
                pageParams,
                criterias
            );
            ...
            Messagebox.show("¿Está seguro de eliminar este registro?", 
                ...
                event -> {
                    if (Messagebox.ON_YES.equals(event.getName())) {
                        try {
                            businessService.removeFromID(MetricSeriesVisualization.class, itemId);
                            log.info("Registro eliminado: ID={}", itemId);
                            logActivity("BORRAR", "V_METRIC_SERIES_VISUALIZATION", itemId, "Eliminado registro ID: " + itemId);
                            loadData();
