    ...
    public void loadData() {
        try {
            ...
            pageResult = businessService.findAllView(
                TrainingResourceUtilization.class,
                pageParams,
                criterias
            );
            ...
            Messagebox.show("¿Está seguro de eliminar este registro?", 
                ...
                event -> {
                    if (Messagebox.ON_YES.equals(event.getName())) {
                        try {
                            businessService.removeFromID(TrainingResourceUtilization.class, itemId);
                            log.info("Registro eliminado: ID={}", itemId);
                            logActivity("BORRAR", "V_TRAINING_RESOURCE_UTILIZATION", itemId, "Eliminado registro ID: " + itemId);
                            loadData();
