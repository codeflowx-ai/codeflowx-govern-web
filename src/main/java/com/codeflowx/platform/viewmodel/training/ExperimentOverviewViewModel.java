...
/**
 * ViewModel para búsqueda y listado de Experiment
...
public class ExperimentOverviewViewModel extends BaseFront<ExperimentOverviewViewModel>{
  ...
  private PageResult<Experiment> pageResult;
  ...
  // ========== Datos ==========
  private List<Experiment> filteredItems = new ArrayList<>();
  ...
  public void loadData() {
    try {
      ...
      pageResult = experimentService.findAll(pageParams, criterias);
      ...
      if (pageResult != null && pageResult.getContent() != null) {
        ...
        // Auditar búsqueda
        logActivity("BUSCAR", "TRNEXPERIMENTS", null,
  ...
  private void loadGlobalMetrics() {
    try {
      ...
      // Ejemplo: List<ExperimentMetricsSummary> metrics =
      // businessService.findAllView(ExperimentMetricsSummary.class);
      ...
      Messagebox.show("¿Está seguro de eliminar este registro?",
          ...
          event -> {
            if (Messagebox.ON_YES.equals(event.getName())) {
              try {
                experimentService.deleteById(itemId);
                log.info("Registro eliminado: ID={}", itemId);
                logActivity("BORRAR", "TRNEXPERIMENTS", itemId, "Eliminado registro ID: " + itemId);
                loadData();
