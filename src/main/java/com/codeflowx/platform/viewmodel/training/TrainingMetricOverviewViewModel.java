...
/**
 * ViewModel para búsqueda y listado de TrainingMetric
...
public class TrainingMetricOverviewViewModel extends BaseFront<TrainingMetricOverviewViewModel>{
  ...
  private PageResult<TrainingMetric> pageResult;
  ...
  // ========== Datos ==========
  private List<TrainingMetric> filteredItems = new ArrayList<>();
  ...
  public void loadData() {
    try {
      ...
      pageResult = trainingMetricService.findAll(pageParams, criterias);
      ...
      if (pageResult != null && pageResult.getContent() != null) {
        ...
        // Auditar búsqueda
        logActivity("BUSCAR", "TRNTRAININGMETRICS", null,
  ...
  private void loadGlobalMetrics() {
    try {
      ...
      // Ejemplo: List<TrainingMetricMetricsSummary> metrics =
      // businessService.findAllView(TrainingMetricMetricsSummary.class);
      ...
      Messagebox.show("¿Está seguro de eliminar este registro?",
          ...
          event -> {
            if (Messagebox.ON_YES.equals(event.getName())) {
              try {
                trainingMetricService.deleteById(itemId);
                log.info("Registro eliminado: ID={}", itemId);
                logActivity("BORRAR", "TRNTRAININGMETRICS", itemId, "Eliminado registro ID: " + itemId);
                loadData();
