...
/**
 * ViewModel para búsqueda y listado de TrainingLog
...
public class TrainingLogOverviewViewModel extends BaseFront<TrainingLogOverviewViewModel>{
  ...
  private PageResult<TrainingLog> pageResult;
  ...
  // ========== Datos ==========
  private List<TrainingLog> filteredItems = new ArrayList<>();
  ...
  public void loadData() {
    try {
      ...
      pageResult = trainingLogService.findAll(pageParams, criterias);
      ...
      if (pageResult != null && pageResult.getContent() != null) {
        ...
        // Auditar búsqueda
        logActivity("BUSCAR", "TRNTRAININGLOGS", null,
  ...
  private Criterias buildCriterias() {
    ...
    if (searchTerm != null && !searchTerm.trim().isEmpty()) {
      Criteria criteria = new Criteria(Operation.AND, Evaluation.LIKE, "source");
  ...
  private void loadGlobalMetrics() {
    try {
      ...
      // Ejemplo: List<TrainingLogMetricsSummary> metrics =
      // businessService.findAllView(TrainingLogMetricsSummary.class);
      ...
      Messagebox.show("¿Está seguro de eliminar este registro?",
          ...
          event -> {
            if (Messagebox.ON_YES.equals(event.getName())) {
              try {
                trainingLogService.deleteById(itemId);
                log.info("Registro eliminado: ID={}", itemId);
                logActivity("BORRAR", "TRNTRAININGLOGS", itemId, "Eliminado registro ID: " + itemId);
                loadData();
