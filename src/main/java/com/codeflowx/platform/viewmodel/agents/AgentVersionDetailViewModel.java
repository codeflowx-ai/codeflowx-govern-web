...
/**
 * ViewModel para DETALLE/EDICIÓN/CREACIÓN de AgentVersion
 */
@Slf4j
@Getter
@Setter
@VariableResolver(DelegatingVariableResolver.class)
public class AgentVersionDetailViewModel extends BaseFront<AgentVersionDetailViewModel> {
    ...
    // ========== Modo de operación ==========
    private String mode;
    private Long idxagentversion;
    private boolean editing = false;
    private String pageTitle = "Detalle";
    
    // ========== Datos ==========
    private AgentVersion currentAgentVersion;
    ...
    public void afterCompose(@ContextParam(ContextType.VIEW) Component view) throws Exception {
        ...
        if (dataParam != null) {
            idxagentversion = Long.valueOf(String.valueOf(dataParam));
        }
        
        log.info("Inicializando AgentVersionDetailViewModel - mode: {}, idxagentversion: {}", mode, idxagentversion);
        
        if ("NEW".equals(mode)) {
            initNew();
        } else if ("LOAD".equals(mode) && idxagentversion != null) {
            loadItem(idxagentversion);
        } else {
            log.error("Modo inválido o falta idxagentversion");
        ...
        unique = new UniqueValidator(currentAgentVersion, businessService);
    }
    
    private void initNew() {
        log.debug("Inicializando nuevo registro");
        currentAgentVersion = new AgentVersion();
        editing = false;
    ...
    private void loadItem(Long id) {
        try {
            ...
            editing = true;
            pageTitle = "Editar: " + currentAgentVersion.getAgtdescription();
        loadAgtstatuss();
            ...
            logActivity("CONSULTA", "AGTAGENTVERSIONS", id, "Consulta: " + currentAgentVersion.getAgtdescription());
    ...
    private boolean validateRequiredFields() {
        ...
        if (currentAgentVersion.getAgtversion() == null || currentAgentVersion.getAgtversion().trim().isEmpty()) {
            errors.append("- Version\n");
        }
        if (currentAgentVersion.getAgtversion() != null && currentAgentVersion.getAgtversion().length() > 50) {
            errors.append("- Version no puede exceder 50 caracteres\n");
        }
        if (currentAgentVersion.getAgtstatus() == null || currentAgentVersion.getAgtstatus().trim().isEmpty()) {
            errors.append("- Status\n");
        }
        if (currentAgentVersion.getAgtcreatedby() == null || currentAgentVersion.getAgtcreatedby().trim().isEmpty()) {
            errors.append("- Created By\n");
        }
        if (currentAgentVersion.getAgtcreatedby() != null && currentAgentVersion.getAgtcreatedby().length() > 255) {
            errors.append("- Created By no puede exceder 255 caracteres\n");
        }
        if (currentAgentVersion.getAgtcreatedat() == null) {
    ...
    public void cancelEdit() {
        ...
        Map<String, Object> params = new HashMap<>();
        params.put("dataParam", idxagentversion);
        params.put("action", Action.LOAD);
        appendPage("plataforma/agents/agents-overview.zul", page.getFellow(IDDESKTOP), params);
    }
