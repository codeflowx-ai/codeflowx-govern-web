...
/**
 * ViewModel para búsqueda y listado de Agent
...
public class AgentOverviewViewModel extends BaseFront<AgentOverviewViewModel> {
    ...
    private PageResult<Agent> pageResult;
    ...
    private String agttypeFilter = "ALL";
    
    // ========== Datos ==========
    private List<Agent> filteredItems = new ArrayList<>();
    ...
    public void loadData() {
        try {
            ...
            pageResult = agentService.findAll(pageParams, criterias
            );
    ...
    private Criterias buildCriterias() {
        ...
        if (searchTerm != null && !searchTerm.trim().isEmpty()) {
            Criteria criteria = new Criteria(Operation.AND, Evaluation.LIKE, "agtname");
        ...
        if (!"ALL".equals(agttypeFilter)) {
            Criteria criteria = new Criteria(Operation.AND, Evaluation.EQUALS, "agttype");
            criteria.setValues(new Object[]{agttypeFilter});
            criterias.addCriteria(criteria);
        }
    ...
    private void loadGlobalMetrics() {
        try {
            ...
            // Ejemplo: List<AgentMetricsSummary> metrics = businessService.findAllView(AgentMetricsSummary.class);
            ...
            Messagebox.show("¿Está seguro de eliminar este registro?", 
                ...
                event -> {
                    if (Messagebox.ON_YES.equals(event.getName())) {
                        try {
                            agentService.deleteById(itemId);
                            log.info("Registro eliminado: ID={}", itemId);
                            logActivity("BORRAR", "AGTAGENTS", itemId, "Eliminado registro ID: " + itemId);
                            loadData();
