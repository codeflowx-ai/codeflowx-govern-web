...
/**
 * ViewModel para DETALLE/EDICIÓN/CREACIÓN de CloudCredential
 */
@Slf4j
@Getter
@Setter
@VariableResolver(DelegatingVariableResolver.class)
public class CloudCredentialDetailViewModel extends BaseFront<CloudCredentialDetailViewModel>{
    ...
    public void afterCompose(@ContextParam(ContextType.VIEW) Component view) throws Exception {
        ...
        if ("NEW".equals(mode)) {
            ...
            loadItem(idxcloudcredential);
        } else {
            log.error("Modo inválido o falta idxcloudcredential");
    ...
    private boolean validateRequiredFields() {
        ...
        if (currentCloudCredential.getInfcredname() == null || currentCloudCredential.getInfcredname().trim().isEmpty()) {
            errors.append("- Credname\n");
        }
        if (currentCloudCredential.getInfcredname() != null && currentCloudCredential.getInfcredname().length() > 255) {
            errors.append("- Credname no puede exceder 255 caracteres\n");
        }
        if (currentCloudCredential.getInfcredtype() == null || currentCloudCredential.getInfcredtype().trim().isEmpty()) {
            errors.append("- Credtype\n");
        }
        if (currentCloudCredential.getInfcredcreatedat() == null) {
            errors.append("- Credcreatedat\n");
        }
    ...
    public void cancelEdit() {
        ...
        Map<String, Object> params = new HashMap<>();
        params.put("dataParam", idxcloudcredential);
        params.put("action", Action.LOAD);
        appendPage("plataforma/infrastructure/infrastructure-overview.zul", page.getFellow(IDDESKTOP), params);
    }
