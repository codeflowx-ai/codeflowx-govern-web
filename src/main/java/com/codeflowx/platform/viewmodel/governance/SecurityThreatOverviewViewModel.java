...
import com.codeflowx.govern.service.governance.SecurityThreatService;
...
/**
 * ViewModel para búsqueda y listado de SecurityThreat
...
public class SecurityThreatOverviewViewModel extends BaseFront<SecurityThreatOverviewViewModel>{

  private static final long serialVersionUID = 1L;
  private static final String IDDESKTOP = "contenedor";

  @WireVariable
  private SecurityThreatService securityThreatService;
  @WireVariable
  ...
  private PageResult<SecurityThreat> pageResult;
  ...
  // ========== Datos ==========
  private List<SecurityThreat> filteredItems = new ArrayList<>();
  ...
  public void loadData() {
    try {
      ...
      pageResult = securityThreatService.findAll(
          pageParams,
          criterias);
      ...
      if (pageResult != null && pageResult.getContent() != null) {
        ...
        // Auditar búsqueda
        logActivity("BUSCAR", "GOVSECURITYTHREATS", null,
  ...
  private Criterias buildCriterias() {
    ...
    if (searchTerm != null && !searchTerm.trim().isEmpty()) {
      Criteria criteria = new Criteria(Operation.AND, Evaluation.LIKE, "threatname");
  ...
  private void loadGlobalMetrics() {
    try {
      ...
      // Ejemplo: List<SecurityThreatMetricsSummary> metrics =
      // businessService.findAllView(SecurityThreatMetricsSummary.class);
      ...
      Messagebox.show("¿Está seguro de eliminar este registro?",
          ...
          event -> {
            if (Messagebox.ON_YES.equals(event.getName())) {
              try {
                securityThreatService.deleteById(itemId);
                log.info("Registro eliminado: ID={}", itemId);
                logActivity("BORRAR", "GOVSECURITYTHREATS", itemId, "Eliminado registro ID: " + itemId);
                loadData();
   ...
   * @param mensaje -- mensaje aclaratorio, ejemplo ha cr;
