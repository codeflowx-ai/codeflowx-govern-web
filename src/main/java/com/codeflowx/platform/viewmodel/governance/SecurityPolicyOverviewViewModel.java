...
/**
 * ViewModel para búsqueda y listado de SecurityPolicy
...
public class SecurityPolicyOverviewViewModel extends BaseFront<SecurityPolicyOverviewViewModel>{
  ...
  private PageResult<SecurityPolicy> pageResult;
  ...
  // ========== Datos ==========
  private List<SecurityPolicy> filteredItems = new ArrayList<>();
  ...
  public void loadData() {
    try {
      ...
      pageResult = securityPolicyService.findAll(
          pageParams,
          criterias);
      ...
      if (pageResult != null && pageResult.getContent() != null) {
        ...
        // Auditar búsqueda
        logActivity("BUSCAR", "GOVSECURITYPOLICIES", null,
  ...
  private Criterias buildCriterias() {
    ...
    if (searchTerm != null && !searchTerm.trim().isEmpty()) {
      Criteria criteria = new Criteria(Operation.AND, Evaluation.LIKE, "policyname");
  ...
  private void loadGlobalMetrics() {
    try {
      ...
      // Ejemplo: List<SecurityPolicyMetricsSummary> metrics =
      // businessService.findAllView(SecurityPolicyMetricsSummary.class);
      ...
      Messagebox.show("¿Está seguro de eliminar este registro?",
          ...
          event -> {
            if (Messagebox.ON_YES.equals(event.getName())) {
              try {
                securityPolicyService.deleteById(itemId);
                log.info("Registro eliminado: ID={}", itemId);
                logActivity("BORRAR", "GOVSECURITYPOLICIES", itemId, "Eliminado registro ID: " + itemId);
                loadData();
