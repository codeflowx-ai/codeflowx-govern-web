...
/**
 * ViewModel para búsqueda y listado de GovernanceMetric
...
public class GovernanceMetricOverviewViewModel extends BaseFront<GovernanceMetricOverviewViewModel>{
  ...
  private PageResult<GovernanceMetric> pageResult;
  ...
  // ========== Datos ==========
  private List<GovernanceMetric> filteredItems = new ArrayList<>();
  ...
  public void loadData() {
    try {
      ...
      pageResult = governanceMetricService.findAll(
          pageParams,
          criterias);
      ...
      if (pageResult != null && pageResult.getContent() != null) {
        ...
        // Auditar búsqueda
        logActivity("BUSCAR", "GOVGOVERNANCEMETRICS", null,
  ...
  private Criterias buildCriterias() {
    ...
    if (searchTerm != null && !searchTerm.trim().isEmpty()) {
      Criteria criteria = new Criteria(Operation.AND, Evaluation.LIKE, "metricname");
  ...
  private void loadGlobalMetrics() {
    try {
      ...
      // Ejemplo: List<GovernanceMetricMetricsSummary> metrics =
      // businessService.findAllView(GovernanceMetricMetricsSummary.class);
      ...
      Messagebox.show("¿Está seguro de eliminar este registro?",
          ...
          event -> {
            if (Messagebox.ON_YES.equals(event.getName())) {
              try {
                governanceMetricService.deleteById(itemId);
                log.info("Registro eliminado: ID={}", itemId);
                logActivity("BORRAR", "GOVGOVERNANCEMETRICS", itemId, "Eliminado registro ID: " + itemId);
                loadData();
   ...
   * @param mensaje -- mensaje aclaratorio, ejemplo hae);
