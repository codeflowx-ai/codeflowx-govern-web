...
public class AutoApprovalAnalyticsOverviewViewModel extends MasterPage {
    ...
    private void loadGlobalMetrics() {
        try {
            ...
            // Ejemplo: List<AutoApprovalAnalyticsMetricsSummary> metrics = businessService.findAllView(AutoApprovalAnalyticsMetricsSummary.class);
            ...
            Messagebox.show("¿Está seguro de eliminar este registro?", 
                ...
                event -> {
                    if (Messagebox.ON_YES.equals(event.getName())) {
                        try {
                            businessService.removeFromID(AutoApprovalAnalytics.class, itemId);
                            log.info("Registro eliminado: ID={}", itemId);
                            logActivity("BORRAR", "V_AUTO_APPROVAL_ANALYTICS", itemId, "Eliminado registro ID: " + itemId);
                            loadData();
