...
/**
 * ViewModel para búsqueda y listado de SecurityMetric
...
public class SecurityMetricOverviewViewModel extends BaseFront<SecurityMetricOverviewViewModel>{
  ...
  private PageResult<SecurityMetric> pageResult;
  ...
  // ========== Datos ==========
  private List<SecurityMetric> filteredItems = new ArrayList<>();
  ...
  public void loadData() {
    try {
      ...
      pageResult = securityMetricService.findAll(
          pageParams,
          criterias);
      ...
      if (pageResult != null && pageResult.getContent() != null) {
        ...
        // Auditar búsqueda
        logActivity("BUSCAR", "GOVSECURITYMETRICS", null,
  ...
  private void loadGlobalMetrics() {
    try {
      ...
      // Ejemplo: List<SecurityMetricMetricsSummary> metrics =
      // businessService.findAllView(SecurityMetricMetricsSummary.class);
      ...
      Messagebox.show("¿Está seguro de eliminar este registro?",
          ...
          event -> {
            if (Messagebox.ON_YES.equals(event.getName())) {
              try {
                securityMetricService.deleteById(itemId);
                log.info("Registro eliminado: ID={}", itemId);
                logActivity("BORRAR", "GOVSECURITYMETRICS", itemId, "Eliminado registro ID: " + itemId);
                loadData();
