...
/**
 * ViewModel para búsqueda y listado de PolicyViolation
...
public class PolicyViolationOverviewViewModel extends BaseFront<PolicyViolationOverviewViewModel>{
  ...
  private PageResult<PolicyViolation> pageResult;
  ...
  // ========== Datos ==========
  private List<PolicyViolation> filteredItems = new ArrayList<>();
  ...
  public void loadData() {
    try {
      ...
      pageResult = policyViolationService.findAll(
          pageParams,
          criterias);
      ...
      if (pageResult != null && pageResult.getContent() != null) {
        ...
        // Auditar búsqueda
        logActivity("BUSCAR", "GOVPOLICYVIOLATIONS", null,
  ...
  private Criterias buildCriterias() {
    ...
    if (searchTerm != null && !searchTerm.trim().isEmpty()) {
      Criteria criteria = new Criteria(Operation.AND, Evaluation.LIKE, "violationtitle");
  ...
  private void loadGlobalMetrics() {
    try {
      ...
      // Ejemplo: List<PolicyViolationMetricsSummary> metrics =
      // businessService.findAllView(PolicyViolationMetricsSummary.class);
      ...
      Messagebox.show("¿Está seguro de eliminar este registro?",
          ...
          event -> {
            if (Messagebox.ON_YES.equals(event.getName())) {
              try {
                policyViolationService.deleteById(itemId);
                log.info("Registro eliminado: ID={}", itemId);
                logActivity("BORRAR", "GOVPOLICYVIOLATIONS", itemId, "Eliminado registro ID: " + itemId);
                loadData();
   ...
   * @param mensaje -- mensaje aclaratorio, ejemplo ha c);
