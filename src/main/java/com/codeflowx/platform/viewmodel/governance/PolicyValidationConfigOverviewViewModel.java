...
/**
 * ViewModel para búsqueda y listado de PolicyValidationConfig
...
public class PolicyValidationConfigOverviewViewModel extends BaseFront<PolicyValidationConfigOverviewViewModel>{
  ...
  private PageResult<PolicyValidationConfig> pageResult;
  ...
  // ========== Datos ==========
  private List<PolicyValidationConfig> filteredItems = new ArrayList<>();
  ...
  public void loadData() {
    try {
      ...
      pageResult = policyValidationConfigService.findAll(
          pageParams,
          criterias);
      ...
      if (pageResult != null && pageResult.getContent() != null) {
        ...
        // Auditar búsqueda
        logActivity("BUSCAR", "GOVPOLICYVALIDATIONCONFIGS", null,
  ...
  private Criterias buildCriterias() {
    ...
    if (searchTerm != null && !searchTerm.trim().isEmpty()) {
      Criteria criteria = new Criteria(Operation.AND, Evaluation.LIKE, "llmmodel");
  ...
  private void loadGlobalMetrics() {
    try {
      ...
      // Ejemplo: List<PolicyValidationConfigMetricsSummary> metrics =
      // businessService.findAllView(PolicyValidationConfigMetricsSummary.class);
      ...
      Messagebox.show("¿Está seguro de eliminar este registro?",
          ...
          event -> {
            if (Messagebox.ON_YES.equals(event.getName())) {
              try {
                policyValidationConfigService.deleteById(itemId);
                log.info("Registro eliminado: ID={}", itemId);
                logActivity("BORRAR", "GOVPOLICYVALIDATIONCONFIGS", itemId, "Eliminado registro ID: " + itemId);
                loadData();
   ...
   * @param mensaje -- mensaje aclaratorio, ejempodel, e);
