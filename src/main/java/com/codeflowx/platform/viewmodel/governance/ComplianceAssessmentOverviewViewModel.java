...
public class ComplianceAssessmentOverviewViewModel extends BaseFront<ComplianceAssessmentOverviewViewModel> {
  ...
  private void loadAllModels() {
    try {
      ...
      PageResult<Model> result = modelService.findAll(
          largePageParams,
          new Criterias());
  ...
  private boolean hasBiasAnalysis(Model model) {
    try {
      ...
      PageResult<ModelBiasAnalysis> result = modelBiasAnalysisService.findAll(
          params, criterias);
  ...
  private boolean hasPerformanceMetrics(Model model) {
    try {
      ...
      PageParams params = PageParams.builder().maxRows(1).pageActual(1).rowActual(0).build();
      PageResult<ModelPerformance> result = modelPerformanceService.findAll(
          params, criterias);
