...
/**
 * ViewModel para DETALLE/EDICIÓN/CREACIÓN de MonitoringAlert
 */
@Slf4j
@Getter
@Setter
@VariableResolver(DelegatingVariableResolver.class)
public class MonitoringAlertDetailViewModel extends BaseFront<MonitoringAlertDetailViewModel>{
    ...
    public void afterCompose(@ContextParam(ContextType.VIEW) Component view) throws Exception {
        ...
        if ("NEW".equals(mode)) {
            ...
            loadItem(idxmonitoringalert);
        } else {
            log.error("Modo inválido o falta idxmonitoringalert");
    ...
    private boolean validateRequiredFields() {
        ...
        if (currentMonitoringAlert.getMonalertname() != null && currentMonitoringAlert.getMonalertname().length() > 255) {
            errors.append("- Alert Name no puede exceder 255 caracteres\n");
        }
        if (currentMonitoringAlert.getMonalerttype() == null || currentMonitoringAlert.getMonalerttype().trim().isEmpty()) {
            errors.append("- Alert Type\n");
        }
        if (currentMonitoringAlert.getMonseverity() == null || currentMonitoringAlert.getMonseverity().trim().isEmpty()) {
            errors.append("- Severity\n");
        }
        if (currentMonitoringAlert.getMonseverity() != null && currentMonitoringAlert.getMonseverity().length() > 50) {
            errors.append("- Severity no puede exceder 50 caracteres\n");
        }
        if (currentMonitoringAlert.getMonstatus() == null || currentMonitoringAlert.getMonstatus().trim().isEmpty()) {
            errors.append("- Status\n");
        }
        if (currentMonitoringAlert.getMontriggeredat() == null) {
            errors.append("- Triggered At\n");
        }
        if (currentMonitoringAlert.getMoncreatedby() == null || currentMonitoringAlert.getMoncreatedby().trim().isEmpty()) {
            errors.append("- Created By\n");
        }
        if (currentMonitoringAlert.getMoncreatedby() != null && currentMonitoringAlert.getMoncreatedby().length() > 255) {
    ...
    public void cancelEdit() {
        ...
        Map<String, Object> params = new HashMap<>();
        params.put("dataParam", idxmonitoringalert);
        params.put("action", Action.LOAD);
        appendPage("plataforma/monitoring/monitoring-overview.zul", page.getFellow(IDDESKTOP), params);
    }
    ...
    public void destroy() {
        ...
        try {
            ...
            if (availableMonalerttypes != null) {
                ...
                availableMonalerttypes = null;
            }
            if (availableMonstatuss != null) {
