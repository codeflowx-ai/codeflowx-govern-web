...
/**
 * ViewModel para DETALLE/EDICIÓN/CREACIÓN de ProjectCostEstimator
 */
@Slf4j
@Getter
@Setter
@VariableResolver(DelegatingVariableResolver.class)
public class ProjectCostEstimatorDetailViewModel extends BaseFront<ProjectCostEstimatorDetailViewModel>{
    ...
    public void afterCompose(@ContextParam(ContextType.VIEW) Component view) throws Exception {
        ...
        if ("NEW".equals(mode)) {
            ...
            loadItem(idxprojectcostestimator);
        } else {
            log.error("Modo inválido o falta idxprojectcostestimator");
    ...
    private boolean validateRequiredFields() {
        ...
        if (currentProjectCostEstimator.getPrjestimationname() == null || currentProjectCostEstimator.getPrjestimationname().trim().isEmpty()) {
            errors.append("- Estimationname\n");
        }
        if (currentProjectCostEstimator.getPrjestimationname() != null && currentProjectCostEstimator.getPrjestimationname().length() > 255) {
            errors.append("- Estimationname no puede exceder 255 caracteres\n");
        }
        if (currentProjectCostEstimator.getPrjperiodstart() == null) {
            errors.append("- Periodstart\n");
        }
        if (currentProjectCostEstimator.getPrjcreatedby() == null || currentProjectCostEstimator.getPrjcreatedby().trim().isEmpty()) {
            errors.append("- Created By\n");
        }
        if (currentProjectCostEstimator.getPrjcreatedby() != null && currentProjectCostEstimator.getPrjcreatedby().length() > 255) {
    ...
    public void cancelEdit() {
        ...
        Map<String, Object> params = new HashMap<>();
        params.put("dataParam", idxprojectcostestimator);
        params.put("action", Action.LOAD);
        appendPage("plataforma/projects/projects-overview.zul", page.getFellow(IDDESKTOP), params);
    }
    ...
    public void destroy() {
        ...
        try {
            ...
            if (subprjbillingdetails != null) {
                ...
                subprjbillingdetails = null;
            }
            subprjbillingdetailsLoaded = false;
            if (subprjinvoices != null) {
                subprjinvoices.clear();
                subprjinvoices = null;
            }
