            ...
            Messagebox.show("¿Está seguro de eliminar este registro?", 
                ...
                event -> {
                    if (Messagebox.ON_YES.equals(event.getName())) {
                        try {
                            businessService.removeFromID(SecurityAuditSummary.class, itemId);
                            log.info("Registro eliminado: ID={}", itemId);
                            logActivity("BORRAR", "V_SECURITY_AUDIT_SUMMARY", itemId, "Eliminado registro ID: " + itemId);
                            loadData();
