package com.codeflowx.web.security.config;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    @Value("${suinsit.security.whilelist}")
    String whilelist;
    @Value("${suinsit.security.blacklist}")
    String blacklist;
    @Override
    protected void configure(HttpSecurity http) throws Exception {
		...
		.requestMatchers(req -> "rmDesktop".equals(req.getParameter("cmd_0"))).permitAll() // allow desktop
																							// cleanup from ZATS
		.antMatchers(whilelist.split(",")).permitAll()
		//.antMatchers("/","/healthz","/actuator/**", "/login.zhtml", "/login.zhtml?logout").permitAll()
		.antMatchers( blacklist.split(",")).authenticated().and().formLogin()
