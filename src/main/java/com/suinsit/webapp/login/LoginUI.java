...
import com.suinsit.discovery.domain.amq.model.GitMessage;
...
public class LoginUI extends MasterPage {
	...
	private void sendQue(long project) {
		GitMessage mess = new GitMessage();
		mess.setBranch("master-pre");
		mess.setIdProject(project);
		rabbitTemplate.convertAndSend("suinsit.octopus.amqp.cloneProject", mess, m -> {
            m.getMessageProperties().setCorrelationId(UUID.randomUUID().toString());
            m.getMessageProperties().getHeaders().put("X-RESPONSEQUE", "git");
            return m;
        });
	}
			...
			if(scroll.getDataList().size()>0) {
				...
				for(Object user:scroll.getDataList()){
				    ...
				    if(!ok) {
				    	...
				    	MessageWin.getMessageBox().show(Labels.getLabel("login.noaccess", "No tienes acceso a está aplicación. Por favor contacta con el administrador"), "Error", Messagebox.OK, Messagebox.ERROR);
				...
				MessageWin.getMessageBox().show(Labels.getLabel("login2.ko", "username/email incorrecto"), "Error", Messagebox.OK, Messagebox.ERROR);
